...
public class LogInScene : BaseScene
{
    ...
    LogInSceneState State
    {
        set
        {
            ...
            if (_state == LogInSceneState.None)
            {

            }
            else if (_state == LogInSceneState.InputingLogInInfo)
            {
            ...
            else if (_state == LogInSceneState.SuccessMakingAccountScripting)
            {
                ContentManager.Instance.ScriptBox.HideSelectBox();
            }
            else if (_state == LogInSceneState.AskingDeleteData)
            {
                ContentManager.Instance.ScriptBox.HideSelectBox();
            }
            else if (_state == LogInSceneState.StartNewGameScripting)
            {
                ContentManager.Instance.ScriptBox.HideSelectBox();
            }
            else if (_state == LogInSceneState.MovingScene)
            {
                ContentManager.Instance.ScriptBox.gameObject.SetActive(false);
            }
            else if (_state == LogInSceneState.MovingGameScene)
            {
                ContentManager.Instance.ScriptBox.HideSelectBox();
                ContentManager.Instance.ScriptBox.gameObject.SetActive(false);
            }
        ...
        if (_packet is S_LogIn)
        {
            ...
            switch (result)
            {
                case LogInResultType.Success:
                    {
                        Managers.Object._myAccountId = _loginAction.GetID();
    ...
    public override void DoNextAction(object value = null)
    {
        switch (_state)
        {
            case LogInSceneState.InputingLogInInfo:
                {
                    string id = _loginAction.GetID();
                    string pw = _loginAction.GetPassword();
            ...
            case LogInSceneState.SuccessLogInScripting:
                {
                    ...
                    if (!_loadingPacket)
                    {
                        ...
                        checkDataPacket.Id = _loginAction.GetID();
                        ...
                        if (inputEvent == Define.InputSelectBoxEvent.SELECT)
                        {
                            ...
                            if (selectBox.GetSelectedBtnData() as string == "Yes")
                            {
                                if (!_loadingPacket)
                                {
                                    ...
                                    createAccountPacket.Id = _loginAction.GetID();
                                    createAccountPacket.Password = _loginAction.GetPassword();
                    ...
                    if (value is Define.InputSelectBoxEvent)
                    {
                        ...
                        if (inputEvent == Define.InputSelectBoxEvent.SELECT)
                        {
                            ...
                            if (selectBox.GetSelectedBtnData() as string == "Yes")
                            {
                                ...
                                State = LogInSceneState.MovingGameScene;
                            }
                            else if (selectBox.GetSelectedBtnData() as string == "No")
                            {
                                List<string> scripts = new List<string>()
            ...
            case LogInSceneState.StartNewGameScripting:
                {
                    ...
                    State = LogInSceneState.MovingScene;
                }
                break;
            case LogInSceneState.MovingScene:
                {
                ...
                break;
            case LogInSceneState.MovingGameScene:
                {
                    ...
                    Managers.Scene.AsyncLoadScene(Define.Scene.Game, () =>
                    {
                        ...
                        Managers.Network.Send(loadGamePacket);
                    });
                }
                break;
